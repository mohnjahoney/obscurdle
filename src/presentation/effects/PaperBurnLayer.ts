...
import { paintBurnTexture } from "../../modes/magnifyingGlass/burnTexturePainter"
...
let nextPaperBurnTextureId = 0

export class PaperBurnLayer {
  ...
  constructor(scene: Phaser.Scene) {
    ...
    const textureKey = `obscurdle-paper-burn-${nextPaperBurnTextureId++}`
    ...
    if (!texture) {
      throw new Error(`Unable to create paper burn texture: ${textureKey}`)
    }
